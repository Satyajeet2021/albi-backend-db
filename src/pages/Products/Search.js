...
export default function Products() {
  ...
  const { id, catId,vendorName } = useParams();
...
console.log("url", { prevUrl, currUrl });
  ...
  const getData = async (page, per_page) => {
    ...
    const vendorNameCaps = vendorName.toUpperCase()
    setLoading(true);
    const data = {
      catAndGender: genderData,
      brands: [vendorNameCaps],
      sizes: sizeCheck,
      gtPrice: gtPrice,
      ltPrice: ltPrice,
      sort: sortPrice,
    ...
    // const res = await axios.post("/multifilter",data) 
    const res = await axios.post("/scrollbox", data)
  ...
  console.log("SET LAST PRODUCT ID ===========>///////////", filterData)
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-4 gap-4 flex flex-wrap mx-auto container product-list">
        ...
        {filterData?.length>0?filterData?.map(function (dt) {
